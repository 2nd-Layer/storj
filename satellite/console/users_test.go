...
func TestGetUnverifiedNeedingReminder(t *testing.T) {
	satellitedbtest.Run(t, func(ctx *testcontext.Context, t *testing.T, db satellite.DB) {
		...
		createdUserNeedingReminder.LastVerificationReminder = previousDay

		err = db.Console().Users().Update(ctx, createdUserNeedingReminder)
		assert.NoError(t, err)

		usersNeedingReminder, err := usersRepo.GetUnverifiedNeedingReminder(ctx, time.Now())
		assert.NoError(t, err)
		...
		users, err := usersRepo.GetUnverifiedNeedingReminder(ctx, previousDay)
		assert.NoError(t, err)
