...
func (obj *pgxImpl) All_User_By_Status_Equal_Number_And_LastVerificationReminder_Less(ctx context.Context,
	user_last_verification_reminder_less User_LastVerificationReminder_Field) (
	rows []*User, err error) {
	defer mon.Task()(&ctx)(&err)

	var __embed_stmt = __sqlbundle_Literal("SELECT users.id, users.email, users.normalized_email, users.full_name, users.short_name, users.password_hash, users.status, users.partner_id, users.user_agent, users.created_at, users.project_limit, users.project_bandwidth_limit, users.project_storage_limit, users.project_segment_limit, users.paid_tier, users.position, users.company_name, users.company_size, users.working_on, users.is_professional, users.employee_count, users.have_sales_contact, users.mfa_enabled, users.mfa_secret_key, users.mfa_recovery_codes, users.signup_promo_code, users.last_verification_reminder FROM users WHERE users.status = 0 AND users.last_verification_reminder < ?")

	var __values []interface{}
	__values = append(__values, user_last_verification_reminder_less.value())
...
func (obj *pgxcockroachImpl) All_User_By_Status_Equal_Number_And_LastVerificationReminder_Less(ctx context.Context,
	user_last_verification_reminder_less User_LastVerificationReminder_Field) (
	rows []*User, err error) {
	defer mon.Task()(&ctx)(&err)
...
func (rx *Rx) All_User_By_Status_Equal_Number_And_LastVerificationReminder_Less(ctx context.Context,
	user_last_verification_reminder_less User_LastVerificationReminder_Field) (
	rows []*User, err error) {
	var tx *Tx
	if tx, err = rx.getTx(ctx); err != nil {
	...
	return tx.All_User_By_Status_Equal_Number_And_LastVerificationReminder_Less(ctx, user_last_verification_reminder_less)
}
...
type Methods interface {
	...
	All_User_By_Status_Equal_Number_And_LastVerificationReminder_Less(ctx context.Context,
		user_last_verification_reminder_less User_LastVerificationReminder_Field) (
		rows []*User, err error)
